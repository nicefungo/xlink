...
### IPC 错误恢复（新增，2026-08-05）

| 计划 | 优先级 | 依赖 | 状态 | 文档 |
|------|--------|------|------|------|
| IPC 自动重连 v1（指数退避 + SIGPIPE 安全） | **P0** | v2.1 | ✅ 已实现（08-04 `d187820`） | [08-ipc-reconnect.md](08-ipc-reconnect.md) |
| 连接状态回调 + 状态机 | P1 | v1 | 规划中 | [08-ipc-reconnect.md](08-ipc-reconnect.md) Phase 2 |
| 退避抖动 + 失败上限 | P1 | v1 | 规划中 | [08-ipc-reconnect.md](08-ipc-reconnect.md) Phase 3 |
| 断线发送缓冲（复用 spsc_queue） | P2 | v1 | 规划中 | [08-ipc-reconnect.md](08-ipc-reconnect.md) Phase 4 |
| 空闲探测 keepalive | P2 | v1 | 规划中 | [08-ipc-reconnect.md](08-ipc-reconnect.md) Phase 5 |
...
| 日期 | 决策 | 背景 |
|------|------|------|
| 2026-08-05 | **IPC 错误恢复 v1 落地 + v2 规划 + 测试告警清理** | 复盘 08-04 已交付的 IPC 自动重连（`try_reconnect` 指数退避 + `sendmsg(MSG_NOSIGNAL)` 修 SIGPIPE + server unlink），新增规划文档 `08-ipc-reconnect.md`，提出 v2 增强：连接状态回调 `xlink_ipc_set_state_cb`、退避抖动 + max_retries 失败上限、断线发送缓冲（复用 spsc_queue，head-drop 保最新）、keepalive 空闲探测；并讨论重连应由专用线程 vs `xlink_run()` 事件循环驱动。index.md 路线图新增该领域表格。**测试告警清理**：修复 test_zc_shm.c 8 处 `xlink_opt_t` missing-field-initializers（改用 designated initializer `.flags=`）与 test_shm_spsc.c 2 处 system() unused-result（捕获返回值），make all 由 10 warnings → 0 warnings，18 test suites ALL PASSED。 |
...
| 2026-07-15 | **Zero-Copy Phase 2 设计深化** | 04-performance.md Phase 2 从 5 行骨架扩展到完整设计文档：核心接口定义（xlink_zc_buf_t、xlink_zc_done_fn、xlink_send_zc、xlink_recv_zc、xlink_zc_poll、xlink_zc_capable）、完成通知机制（环形完成队列 + epoll 错误队列）、SHM 零拷贝（元数据传递，预期 3-10× 提速）、TCP MSG_ZEROCOPY（SO_ZEROCOPY + sendmsg、Linux 4.14+）、File splice/copy_file_range（page cache 操作）。新增 3 个设计决策：批量化 API 独立不侵入 xlink_send()、零拷贝统一 API 后端内部分化、所有权通过异步回调管理。Phase 2 从 5 步扩展到 8 个详细实现步骤。make all 0 warnings，make test 36/36 ALL PASS。|
...
| 2026-07-09 | **TLS per-client 实现（Phase 2.1）** | src/tcp_backend.c 和 src/tls.c 变更。Server 模式每个客户端现在拥有独立的 SSL 对象（共享 SSL_CTX）。`tcp_priv_t` 新增 `client_tls[]` 数组与 `client_fds` 并行维护。`tls.c` 新增 `tls_clone_for_client()` 和 `xlink_tls_free_client_ssl()`。`recv_multi()` 和 `send_to_all()` 重写 TLS 路径，支持多客户端独立加密通道。`add_client`/`remove_client`/`tcp_backend_close` 完整生命周期管理。非 TLS 构建零影响（`#ifdef XLINK_HAS_TLS` 保护）。make all 0 warnings，make test 32/32 ALL PASS。 |
| 2026-07-07 | 第 97 轮 — 竞品对比分析 | 新增 `07-competitive-analysis.md`：xlink vs nanomsg/nng/ZeroMQ 功能/性能/架构对比。xlink 在传输层多样性（6种后端 vs 3-4种）、裸 Pipe 延迟（0.004ms vs ~0.01ms）、SHM 零拷贝（nng/ZMQ 不支持）、io_uring 支持（唯一）方面差异化。在消息模式层（pub/sub、req/rep）和线程安全方面有 gap。分析为未来路线图提供参考。make all 0 warnings，make test 32/32 ALL PASS。 |
...
- [性能基准测试报告](06-perf-benchmarks.md)
- [竞品对比分析](07-competitive-analysis.md)
